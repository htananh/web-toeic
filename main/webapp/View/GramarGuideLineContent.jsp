...
<head>
<meta charset="UTF-8">
<title>GrammarGuideLine Content</title>
<!-- ===============================================-->
    <!--    Favicons-->
    <!-- ===============================================-->
	...
	function comment(){
		...
		var url="CommentController?cmtgrammarcontent="+cmtgrammarcontent+"&memberid="+memberid+"&grammarguidelineid="+grammarguidelineid;
        ...
        <div class="container">
          <div class="row">
            <div >
              ...
              <h1 class="mb-6">${list.grammarname}</h1>
              </c:forEach>
            </div>
          </div>
        </div>
      ...
      <section style="margin-top:-21.5rem">
        ...
        <div class="container">
          ...
          <c:forEach var="list" items="${ListContent}">
              ...
              <img class="w-50" src="imageUpload/${list.grammarimage}" alt="..." />
              <br>
              <br>
              <br>

<c:set var="characterDB2" value="**" />
<c:set var="characterHTML2" value="<b>" />
<c:set var="characterDB3" value="###" />
<c:set var="characterHTML3" value="<h1>" />

<c:set var="content" value="${fn:replace(list.content, characterDB1, characterHTML1)}" />

<c:choose>
    <c:when test="${fn:indexOf(content, characterDB2) != -1}">
        <c:forEach var="text" items="${fn:split(content, characterDB2)}" varStatus="status">
            <c:choose>
                <c:when test="${status.index % 2 == 1}">
                    <c:out value="${text}" escapeXml="false" />
                </c:when>
                <c:otherwise>
                    <b><c:out value="${text}" escapeXml="false" /></b>
                </c:otherwise>
            </c:choose>
        </c:forEach>
    </c:when>
              ...
              <form name="FormComment" class="row g-0 align-items-center">
                <div class="col">
                  <div class="input-group-icon">
                    <input class="form-control form-little-squirrel-control" type="text" name="cmtgrammarcontent" placeholder="Text "  /><i class="fas fa-comment input-box-icon"></i>
                  </div>
                </div>
                <div class="col-auto">
